package com.gintong.httpclient;
...
import com.gintong.utils.Utils;
import com.gintong.httpclient.builder.HACB;
...
public class HttpAsyncClientUtil{
    ...
    public static CloseableHttpAsyncClient create(String url) throws HttpProcessException{
      ...
      *
      * @Author: Frame
      * @Data:   2016/7/19 14:21
      * @Param:  url
      * @Param:  hostOrIP
      * @Param:  port
      */
    public static CloseableHttpAsyncClient create(String url,String hostOrIP,int port ) throws HttpProcessException{
        if(url.toLowerCase().startsWith("https://")){
            return HACB.custom().proxy(hostOrIP,port).ssl().build();
        }else{
            return HACB.custom().proxy(hostOrIP,port).build();
        }
    }
    ...
    private static void execute(HttpConfig config) throws HttpProcessException {
        ...
        if((null==config.hostOrIP())||(0==config.port())){
            if(config.asynclient()==null){//检测是否设置了client
                config.asynclient(create(config.url()));
            }
        }else {
            if(config.asynclient()==null){//检测是否设置了client
                config.asynclient(create(config.url(),config.hostOrIP(),config.port()));
            }
        }
...
//                HttpEntity entity = Utils.map2List(nvps, config.map(), config.inenc());
                ...
                //修改  如果没有map为空就选择json方式
                HttpEntity entity;
                if(null==config.map()){
                    entity = Utils.map2json( config.jsonobj(), config.inenc());
                }else {
                    entity = Utils.map2json( config.map(), config.inenc());
                }
//                HttpEntity entity = Utils.map2json( config.map(), config.inenc());
                //设置参数到请求对象中
            ...
            client.execute(request, new FutureCallback<HttpResponse>() {
                ...
                public void completed(HttpResponse resp) {
                    try {
                        if(out == null){
//                           resbody 写回 config
//                            config.resbody(handler.completed(fmt2String(resp, encoding)).toString());
...
//        HttpAsyncClientUtil.create(url).
        ...
        HttpAsyncClientUtil.send(HttpConfig.custom().url(url).handler(handler).hostOrIP("127.0.0.1").port(8888).headers(HttpHeader.custom().keepAlive("false").build()));
    }
}