...
import com.gintong.httpclient.builder.HCB;
...
public class HttpClientUtil{
     ...
     * @Param:  port
     */
    public static HttpClient create(String url, String hostOrIP, int port ) throws HttpProcessException{
        if(url.toLowerCase().startsWith("https://")){
            return HCB.custom().proxy(hostOrIP,port).ssl().build();
        }else{
            return HCB.custom().proxy(hostOrIP,port).build();
        }
    }
    ...
    private static HttpResponse execute(HttpConfig config) throws HttpProcessException {
        ...
        if((null==config.hostOrIP())||(0==config.port())){
            ...
            if(config.asynclient()==null){//检测是否设置了client
                config.client(create(config.url(),config.hostOrIP(),config.port()));
            }
        }
